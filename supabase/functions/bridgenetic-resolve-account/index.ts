...
async function headersFor(mode: BridgeneticAuthMode, body: string) {
...
async function callBridgenetic(body: string) {
  ...
  for (const mode of attempts) {
    const res = await fetch(`${BASE}/accounts/resolve`, { method: "POST", headers: await headersFor(mode, body), body });
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!account_number || !bank_code) {
      return jsonResponse({ error: "account_number and bank_code required" }, 400);
    }
    const body = JSON.stringify({ account_number, bank_code });
    const result = await callBridgenetic(body);
    if (!result.ok) {
      const msg = result.json?.error?.message || result.json?.message || "Could not resolve account";
      console.error("Bridgenetic resolve failed", { status: result.status, message: msg, authMode: result.mode });
      return jsonResponse({
        status: "error",
